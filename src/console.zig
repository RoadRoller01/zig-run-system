...
const Shell = struct {
    ...
    pub fn handleInput(self: *@This()) !?[]const u8 {
...
pub fn handleStdin(self: *Console) !?Event {
    const maybe_input = try self.shell.handleInput();
    ...
    if (maybe_input) |user_input| {
        ...
        if (self.runCommand(&args)) |maybe_event| {
...
fn runCommand(
    ...
    if (args.next()) |cmd| {
        if (std.mem.eql(u8, cmd, "help")) {
            return Command.help.run(self, args);
        }
...
pub const Command = struct {
    const NoContext = enum { clear, history, logs, poweroff, reboot, exit };
    ...
    const history = struct {
        ...
        fn run(console: *Console, _: *ArgsIterator) !?Event {
            writeAll("\n");
